#coding:utf-8
"""
ID:          utilites.gstat.file_blobs.missing_files
TITLE:       Check file blobs statistics.
DESCRIPTION: Some files not exist.
             We delete half files after their creation
NOTES:
"""
...
NEW (128)
    'FILES' field: 
        ...
        Missing files' count: {NEW_BLOB_QNT//2}
...
def test_1(act: Action, gstat_helpers, conf: ConfigManager, new_config: Path, tmp_path: Path):   
    ...
    # Delete half of blob files from each table
    for blob_f in ('new', 'old'):
        blobs = list(test_dir.glob(f'**/{blob_f}_blob*.txt'))
